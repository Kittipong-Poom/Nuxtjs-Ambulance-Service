    ...
    <v-card>
      ...
      <form @submit.prevent="save">
        ...
        <v-col cols="12" md="6">
          ...
          <v-text-field disabled v-model="editedItem.other" label="เพิ่มเติม" outlined :rules="[rules.other]"></v-text-field>
  ...
  <v-btn text color="primary" @click="$refs.menu.save(date)">
    ตกลง
          ...
          <v-menu ref="menu" v-model="menu" :close-on-content-click="false" :return-value.sync="editedItem.service_date"
            transition="scale-transition" offset-y min-width="auto">
            <template v-slot:activator="{ on, attrs }">
              <v-text-field v-model="formattedDate" label="วันที่นัดหมาย" outlined prepend-icon="mdi-calendar" readonly
                v-bind="attrs" v-on="on" clearable></v-text-field>
            </template>
        ...
        <!-- Time -->
        <v-col cols="12" md="6">
          <v-text-field v-model="editedItem.time" label="เวลา" outlined prepend-icon="mdi-clock-outline"></v-text-field>
        </v-col>
        </v-row>
          <!-- Status -->
          <v-select v-model="editedItem.status_case_id" label="สถานะ" outlined :items="items_status" item-text="casestatus_name"
