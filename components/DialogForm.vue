            ...
            <v-row>
              <v-col cols="12" md="6">
                ...
                <v-text-field v-model="editedItem.hn" label="HN (Hospital Number)*"
                  prepend-inner-icon="mdi-hospital-building" outlined :rules="[rules.hn]" ref="hn"></v-text-field>
              </v-col>
              <v-col cols="12" md="6">
                <div class="input-container">
                  <!-- อายุ -->
                  <v-select v-model="editedItem.ages_id" label="อายุ" :items="items_ages" outlined item-text="age_name"
                    item-value="age_id" prepend-inner-icon="mdi-account-group" return-object></v-select>
                </div>
              </v-col>
              <v-col cols="12">
                <!-- เพศ -->
            ...
            <v-row>
              <v-col cols="12" md="6">
                ...
                <v-text-field class="custom-icon-color" v-model="editedItem.lati" prepend-inner-icon="mdi-map-marker"
                  label="ละติจูด*" outlined :rules="[rules.lati]" ref="lati"></v-text-field>
              </v-col>
              <v-col cols="12" md="6">
                <!-- พิกัด -->
                <v-text-field class="custom-icon-color" v-model="editedItem.longi" prepend-inner-icon="mdi-map-marker"
                  label="ลองติจูด*" outlined :rules="[rules.longi]" ref="longi"></v-text-field>
              </v-col>
            ...
            <v-row>
              <v-col cols="12" md="6">
                ...
                <v-select v-model="editedItem.tracking_patient_id" prepend-inner-icon="mdi-account-injury-outline"
                  ...
                  item-value="tracking_id"></v-select>
              </v-col>
              <v-col cols="12" md="6">
                <!-- เลือกประเภท -->
                <v-select v-model="editedItem.type_patient_id" prepend-inner-icon="mdi-clipboard-list-outline"
                  ...
                  item-value="type_patient_id"></v-select>
              </v-col>
            </v-row>
            <v-row>
              <v-col cols="12">
                ...
                <v-text-field v-model="editedItem.other" prepend-inner-icon="mdi-information"
                  label="กรอกรายละเอียดเพิ่มเติม" outlined :rules="[rules.other]"></v-text-field>
              </v-col>
            </v-row>
          </v-container>
        </v-card-text>
